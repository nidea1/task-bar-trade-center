...
func (resolver *hoveredItemAOBResolver) read(processHandle uintptr, moduleBase uintptr, layout GameLayout) (int32, string, int32, bool) {
...
func (resolver *tooltipAOBResolver) resolve(label string, processHandle uintptr, moduleBase uintptr, pattern AOBPattern, offsets []uintptr) (uintptr, bool, string) {
	...
	if resolver.resolvedBase != 0 {
		if address, ok, trace := resolveTooltipPointerChain(label+" AOB", resolver.resolvedBase, offsets); ok {
	...
	for _, candidate := range resolver.candidates {
		address, ok, trace := resolveTooltipPointerChain(label+" AOB", candidate, offsets)
		if !ok {
			continue
		}
